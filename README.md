...
| `immediate_surfaces` | bool (0/1) | `0` | Experimental (render-core migration A/B switch, removed in a later step): `1` draws the strip thumbnails immediately inside their chrome phase instead of queueing them into the compositor's render pass. Pixel output is identical; flip it live to compare or roll back |
